...
use App\Jobs\ReportStockInStoresDetalle;
use App\Jobs\ReportSaleItemsByVendor;
use App\Jobs\ReportExportInventory;
use App\Jobs\ReportExportInventoryGeneral;
...
class AdvancereportsController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach($_roles as $item){
            $roles[$item->resource_code] = true;
        }
        return View::make('advancereports.index', ['roles' => $roles]);
    }
    ...
    public function customersInactive(Request $request){
        ...
        if(count($data) > 0){
            ...
            foreach($clients as $client){
                if(strtotime($client->invoice_date) > strtotime($to_date)){ continue; }
                    $result[$client->id] = [
                        'id' => $client->id,
        ...
        if(count($data) > 0){
            ...
            for($i = 0; $i <= $monthAgo; $i++){
                ...
                $invoices = DB::table('invoices')
                    ...
                    ->join('accounts', 'accounts.id', '=', 'clients.account_id')
                    ->select(
                        DB::raw('SUM(invoices.total_cost) AS total_cost'), DB::raw('SUM(invoices.amount) AS amount'),
                        'clients.name', 'clients.address1', 'clients.work_phone', 'clients.phone', 'invoices.client_id',
                        'clients.company_name', 'clients.seller_id', 'clients.points', 'clients.task_date', 'accounts.name as account_name'
                    )
                    ->where('invoices.account_id', '<>', 6)->where('invoice_type_id', 1)
                    ->where('accounts.exclude', 0)
                    ->whereDate('invoices.invoice_date', '>=', $from_date)
    ...
    public function salesPerClient(Request $request){
        ...
        if(count($data) > 0){
            ...
            $result = []; $totals = ['account' => 'Total', 'invoice_count' => 0, 'client_count' => 0, 'client_amount' => 0, 'invoice_amount' => 0];
    ...
    public function salesBySeller(Request $request){
        ...
        if(count($data) > 0){
            ...
            $to_date = $data['to_date'];
            $to_date = date('Y-m-d', strtotime($to_date. ' + 1 days'));
            $columns = ['Vendedor', 'Tienda', 'Zona', 'Cliente', 'Telefono', 'Saldo', 'Total Facturado', 'Costo Total', 'Devolucion', 'Tipo', 'Factura', 'Fecha de Factura', 'Fecha de Pago', 'Comision', 'Perfil'];
            ...
            if($rows == 1){
                dispatch((new ReportSalesBySeller($nameFile, $reportProcessId, $currentStores, $from_date, $to_date))->delay(30));
            }else{
                $count = 1;
                foreach (array_chunk($currentStores, 4) as $chunkStores){
                    dispatch((new ReportSalesBySeller($nameFile, $reportProcessId, $chunkStores, $from_date, $to_date))->delay(30 * $count));
                    $count = $count+1;
                };
            };
        }
    ...
    public function salesByVendor(Request $request){
        ...
        if(count($data) > 0){
            ...
            $invoices = DB::table('invoices')->join('invoice_items', 'invoice_items.invoice_id', '=', 'invoices.id')
                ...
                ->join('accounts', 'accounts.id', '=', 'invoices.account_id')
                ->select(
                    'invoice_items.qty', 'invoice_items.cost', 'invoice_items.product_cost',
                    'vendors.name', 'products.vendor_id',
        ...
        if(count($data) > 0){
            ...
            $invoices = DB::table('invoices')->join('accounts', 'invoices.account_id', '=', 'accounts.id')
                    ->select(
                        ...
                        DB::raw('SUM(invoices.oil_amount) as oil_amount'), DB::raw('SUM(invoices.replacement_amount) as replacement_amount'),
                        DB::raw('SUM(invoices.total_cost) as total_cost'),
                        DB::raw('SUM(invoices.amount) as sale_amount'),'accounts.name as account', 'accounts.id as account_id', 'invoices.invoice_date'
                    )->where('accounts.exclude', 0)->where('invoice_type_id', 1)
                    ->whereDate('invoices.invoice_date', '>=', $from_date)->whereDate('invoices.invoice_date', '<', $to_date)
		    ...
		    $result = []; $totals = ['sale_amount' => 0, 'sale_cost' => 0];
            foreach($invoices as $invoice){
        ...
        if(count($data) > 0){
            ...
            $invoices = DB::table('invoices')->join('accounts', 'invoices.account_id', '=', 'accounts.id')
                ->select(
                    ...
                    DB::raw('SUM(invoices.oil_amount) as oil_amount'), DB::raw('SUM(invoices.replacement_amount) as replacement_amount'),
                    DB::raw('SUM(invoices.total_cost) as total_cost'),
                    DB::raw('SUM(invoices.amount) as sale_amount'),'accounts.name as account', 'accounts.id as account_id',
                    DB::raw('DATE_FORMAT(invoices.invoice_date, "%Y-%m") as invoice_month')
                )->where('accounts.exclude', 0)->where('invoice_type_id', 1)
                ->whereDate('invoices.invoice_date', '>=', $from_date)->whereDate('invoices.invoice_date', '<', $to_date)
    ...
    public function stockByVendor(Request $request){
        ...
        if(count($data) > 0){
            ...
            for ($i=1; $i < $productCount; $i=$i+400) {
                ...
                dispatch((new ReportStockByVendor($chunk, $nameFile, $group, $_vendors, $_accounts, $is_root, $reportProcessId, $vendor))->delay(15 * $count));
                $count = $count+1;
            }
	    }
    ...
    public function saleItemsByVendor(Request $request){
        ...
        if(count($data) > 0){
            ...
            $vendor_id = (isset($data['vendor_id']) && $data['vendor_id'] > 0) ? $data['vendor_id'] : null;
            if(is_null($vendor_id)){
                Session::flash('message', "No a seleccionado ningun proveedor");
                return redirect()->back();
            }
            $vendorName = $_vendors[$vendor_id]->name;
            ...
            $columns = [
                'Codigo', 'Producto', 'Relacion', 'Proveedor', 'Cliente', 'Factura', 'Vendedor', 'Tipo de pago', 'Costo', 'Precio', 'Cantidad Global',
                'Cantidad en Bodega', 'Equivalencias en Bodega', 'Equivalencias Globales', 'Ventas Equivalencias', 'Unidades Vendidas', 'Unidades Devueltas',
                'Porcentaje ult. 6 meses', 'Porcentaje Equivalencias ult. 6 meses', 'Costo Total', 'Monto Total', 'Monto Reembolsado', 'Monto Final', 'Tienda'
            ];
            ...
            $nameFile = 'saleitems_by_vendor_'.$vendorName.'_'.$currentDate[0].$currentTime.'.csv';
            ...
            $rows = isset($currentProducts) ? count($currentProducts) : 1;
            
            $reportProcess = new ReportProcess;
            $reportProcess->file = $nameFile;
            $reportProcess->report = 'saleitems_by_vendor';
            $reportProcess->status = 0;
            $reportProcess->rows = intval(ceil($rows / 50));
            $reportProcess->count_rows = 0;
            $reportProcess->save();
            $reportProcessId = $reportProcess->id;

            $count = 1;
            foreach (array_chunk($currentProducts, 50) as $chunkProducts){
                dispatch((new ReportSaleItemsByVendor($chunkProducts, $nameFile, $group, $_vendors, $_accounts, $is_root, $reportProcessId, $vendor_id, $dateArrays))->delay(15 * $count));
                $count = $count+1;
            }
        }
        $reportProcess = ReportProcess::where('report','saleitems_by_vendor')->orderBy('id', 'DESC')->take(30)->get();
        return view('advancereports.saleitems_by_vendor', ['reportProcess' => $reportProcess, 'vendors' => $_vendors]);
    }
    ...
    public function salesByProduct(Request $request){
        ...
        if(count($data) > 0){
            ...
            $invoices = DB::table('invoices')
				...
				->join('accounts', 'accounts.id', '=', 'invoices.account_id')
				->select([
                    'products.product_key', 'products.notes', 'products.category_id',
                    'invoice_items.qty', 'invoices.account_id', 'invoice_items.cost', 'products.relation_id'
                ])
				->where('accounts.exclude', 0)->whereNull('invoice_items.deleted_at')
            ...
            $productsGlobal = Product::
                whereIn('product_key', collect($invoices)->pluck('product_key')->toArray())
            ...
            $columns = [
                'Codigo', 'Descripcion', 'Cantidad en Bodega',
                'Unidades Vendidas', 'Monto Vendido', 'Categoria',
                'Equivalencias', 'Cantidad de Equivalencias Vendidas', 'Cantidad Global'
            ];
    ...
    public function productsUnderWholesalePrice(Request $request){
        ...
        if(count($data) > 0){
            ...
            $employees = Employee::get()->keyBy('id');
            foreach($invoices as $item){
			    if(floatval($item->cost) >= floatval($item->wholesale_price)){ continue; }
                $result[] = [
                    'product_key' => $item->product_key,
                    'notes' => $item->notes,
                    'cost' => $item->cost,
    ...
    public function productsUnderNormalPrice(Request $request){
        ...
        if(count($data) > 0){
            ...
            $employees = Employee::get()->keyBy('id');
            foreach($invoices as $item){
                if(floatval($item->cost) >= floatval($item->normal_price)){ continue; }
                $result[] = [
                    'product_key' => $item->product_key,
                    'notes' => $item->notes,
                    'cost' => $item->cost,
    ...
    public function exportInventoryGeneral(Request $request){
        $data = $request->all();
        if(count($data) > 0){
            $currentStores = Account::where('accounts.exclude', 0)->pluck('id')->toArray();
            $userIsCost = Auth::user()->realUser()->_can('cost');
            ...
            $columns = array_merge($columns,[
                'Precio Detalle', 'Precio Mayorista', 'Precio especial', 'Cantidad', 'Cantidad Global',
                'Equivalencias en Tienda', 'Cantidad Global de Equivalencias',
                'Promedio Cantidad Vendida Ult. 6 Meses', 'Promedio Equivalencia Vendida Ult. 6 Meses',
                'Cantidad Minima', 'Cantidad Maxima', 'Codigo de Equivalencia', 'Proveedor',
                'Marca', 'Categoria', 'Sub Categoria', 'Rotacion'
            ]);
            ...
            $nameFile = 'export_inventory_general_'.$currentDate[0].$currentTime.'.csv';
            $bom = "\xEF\xBB\xBF";
            ...
            $reportProcess = new ReportProcess;
            $reportProcess->file = $nameFile;
            $reportProcess->report = 'export_inventory_general';
            $reportProcess->status = 0;
            $reportProcess->count_rows = 0;
            $reportProcess->rows = intval(ceil($rows / 4));
            $reportProcess->save();
            $reportProcessId = $reportProcess->id;
            
            $count = 1;
            foreach (array_chunk($currentStores, 4) as $chunkStores){
                dispatch((new ReportExportInventoryGeneral($nameFile, $reportProcessId, $chunkStores, $userIsCost))->delay(15 * $count));
                $count = $count+1;
            };
        }
        $reportProcess = ReportProcess::where('report','export_inventory_general')->orderBy('id', 'DESC')->take(30)->get();
        return view('advancereports.export_inventory_general', ['reportProcess' => $reportProcess]);
    }
    ...
    public function exportInventory(Request $request){
        ...
        if(count($data) > 0){
            ...
            $user = Auth::user();
            $accountName = isset($user->account) ? $user->account->name : '';
            ...
            $columns = array_merge($columns,
                [
                    'Precio de venta', 'Precio de mayorista', 'Precio especial', 'Precio dos', 'Precio tres',
                    'Cantidad', 'Cantidad equivalencia', 'Cantidad Minima', 'Cantidad Maxima', 'Cantidad Global',
                    'Promedio Cantidad Vendida Ult. 6 Meses', 'Promedio Equivalencia Vendida Ult. 6 Meses',
                    'Marca', 'Categoria', 'Sub Categoria', 'Rotacion', 'Bodega', 'Ubicacion', 'Equivalencia', 
                    'Aplicaciones', 'Proveedor', 'Imagen', 'Dias Transcurridos', 'Fecha de Inicio',
                    'Fecha de factura', 'Fecha de inventario'
                ]
            );
            ...
            $nameFile = 'inventory_'.$accountName.'_'.$currentDate[0].$currentTime.'.csv';
            $bom = "\xEF\xBB\xBF";
            $fp = fopen($nameFile, 'w');
            fputcsv($fp, $columns, ';');
            fclose($fp);
            ...
            $reportProcess = new ReportProcess;
            $reportProcess->file = $nameFile;
            $reportProcess->report = 'inventory';
            $reportProcess->status = 0;
            $reportProcess->rows = $rows;
            $reportProcess->count_rows = 0;
            $reportProcess->save();
            ...
            for ($i=1; $i < $productCount; $i=$i+400) {
                ...
                $init = ($i == 1) ? 0 : $i;
                $chunk = ['skip' => $init, 'take' => $i+399];
                dispatch((new ReportExportInventory($chunk, $nameFile, $reportProcessId, $user_account_id, $userIsCost))->delay(15 * $count));
                $count = $count+1;
            }
        }
        $reportProcess = ReportProcess::where('report','inventory')->orderBy('id', 'DESC')->take(30)->get();
        return view('advancereports.export_inventory', ['reportProcess' => $reportProcess]);
    }
    ...
    public function exportClients(Request $request){
        ...
        if(count($data) > 0){
            ...
            $reportProcessId = $reportProcess->id;
            if($rows == 1){
                dispatch((new ReportExportClients($nameFile, $reportProcessId, $currentStores))->delay(15));
            }else{
                $count = 1;
                foreach (array_chunk($currentStores, 4) as $chunkStores){
                    dispatch((new ReportExportClients($nameFile, $reportProcessId, $chunkStores))->delay(15 * $count));
                    $count = $count+1;
                };
            };
        }
    ...
    public function saleTransferProducts(Request $request) {
        ...
        if(count($data) > 0){
            ...
            $invoices = DB::table('invoices')
              ...
              ->join('accounts', 'accounts.id', '=', 'invoices.account_id')
              ->select(
                'products.product_key', 'products.notes', 'products.qty as product_qty',
                'invoice_items.qty', 'invoice_items.cost', 'invoice_items.product_cost',
                'products.relation_id', 'products.location', 'products.vendor_id',
                'products.category_id', 'products.sub_category_id', 'products.rotation_id'
              )->where('accounts.exclude', 0)->whereNull('invoice_items.deleted_at')
              ->whereDate('invoices.invoice_date', '>=', $from_date)
              ->whereDate('invoices.invoice_date', '<=', $to_date)
              ->where('invoices.invoice_type_id', 1)
            ...
            $transfers = DB::table('transfers')
              ...
              ->select(
                'products.product_key', 'products.notes', 'products.qty as product_qty',
                'products.location', 'products.relation_id', 'transfer_items.qty',
                'products.vendor_id', 'products.category_id', 'products.sub_category_id',
    ...
    public function mostSelledProducts(Request $request) {
        ...
        if(count($data) > 0){
            ...
            $items = DB::table('invoices')
              ...
              ->join('accounts', 'accounts.id', '=', 'invoices.account_id')
              ->select(
                'invoice_items.product_id', 'invoice_items.product_key', 'invoice_items.notes',
                ...
                'invoices.account_id', 'products.qty as product_qty', 'products.relation_id',
                'products.vendor_id', 'invoice_items.product_cost'
              )->where('accounts.exclude', 0)->whereNull('invoice_items.deleted_at')
              ->whereDate('invoices.invoice_date', '>=', $from_date)->whereDate('invoices.invoice_date', '<=', $to_date)
              ->where('invoices.invoice_type_id', 1);
            if($account_id > 0 ){
            ...
            $products = Product::select(
              'id', 'product_key', 'qty',
              'category_id', 'sub_category_id', 'rotation_id'
    ...
    public function inputEntries(Request $request){
        ...
        if(count($data) > 0){
            ...
            $reportProcessId = $reportProcess->id;
            if($rows == 1){
                dispatch((new ReportInputEntries($nameFile, $reportProcessId, $currentStores, $from_date, $to_date))->delay(15));
            }else{
                $count = 1;
                foreach (array_chunk($currentStores, 4) as $chunkStores){
                    dispatch((new ReportInputEntries($nameFile, $reportProcessId, $chunkStores, $from_date, $to_date))->delay(15 * $count));
                    $count = $count+1;
                };
            };
    ...
    public function exportInvoices(Request $request){
        ...
        if(count($data) > 0){
            ...
            $reportProcessId = $reportProcess->id;
            if($rows == 1){
                dispatch((new ReportExportInvoices($nameFile, $reportProcessId, $currentStores, $from_date, $to_date, $filter))->delay(15));
            }else{
                $count = 1;
                foreach (array_chunk($currentStores, 4) as $chunkStores){
                    dispatch((new ReportExportInvoices($nameFile, $reportProcessId, $chunkStores, $from_date, $to_date, $filter))->delay(15 * $count));
                    $count = $count+1;
                };
            };
	    }
    ...
    public function exportInventoryNotLocations(){
        ...
        $products = DB::table('products')
            ...
            ->get();
        $result = Product::
                select('product_key',DB::raw('SUM(qty) as qty_total'))
	...
	fputcsv($fp, $columns, ';');
	    ...
	    if(!$quote){ continue; }
	    if($invoice->amount == $quote->amount){ continue; }
    ...
    public function quotedProducts(Request $request){
        ...
        if(count($data) > 0){
                ...
                if($rows == 1){
                        dispatch((new ReportQuotedProformProducts($nameFile, $reportProcessId, $currentStores, $from_date, $to_date, 'quoted'))->delay(15));
                }else{
                        $count = 1;
                        foreach (array_chunk($currentStores, 4) as $chunkStores){
                                dispatch((new ReportQuotedProformProducts($nameFile, $reportProcessId, $chunkStores, $from_date, $to_date, 'quoted'))->delay(15 * $count));
                                $count = $count+1;
                        };
                };
        }
    ...
    public function proformProducts(Request $request){
        ...
        if(count($data) > 0){
                ...
                if($rows == 1){
                        dispatch((new ReportQuotedProformProducts($nameFile, $reportProcessId, $currentStores, $from_date, $to_date, 'proform'))->delay(15));
                }else{
                        $count = 1;
                        foreach (array_chunk($currentStores, 4) as $chunkStores){
                                dispatch((new ReportQuotedProformProducts($nameFile, $reportProcessId, $chunkStores, $from_date, $to_date, 'proform'))->delay(15 * $count));
                                $count = $count+1;
                        };
                };
        }
    ...
    public function vouchersDiscounts(Request $request){
        ...
        if(isset($data['expoor_clients']) && $data['expoor_clients'] == 1){
                $result = DB::table('clients')
                                ...
                                'clients.percentage_vouchers', 'clients.amount_vouchers_kms',
                                'clients.phone', 'accounts.name as account')
        ...
        $result = [];
                if(count($data) > 0){
                        ...
                        $query = DB::table('expenses as ex')
                                ...
                                ->select('ex.*', 'ec.name as categorie_name', 'es.name as subcategorie_name', 'es.code as code', 'fa.name as fa_name', 
                                        'accounts.name as account_name', 'users.first_name as user_name', 'users.last_name as user_name2',
                                        'em.first_name as employee_name', 'em.last_name as employee_name2')
                ...
                return view('advancereports.cash_count_expenses', ['result' => $result, 'stores' => $stores, 
                'categories' => $categories, 'subcategories' => $subcategories, 'financeAccounts' => $financeAccounts]);
    }
    ...
    public function expensesAllCashCount(Request $request){
        ...
        $results = [];
                ...
                return view('advancereports.cash_count_expenses_all', ['results' => $results, 'stores' => $stores, 
                'categories' => $categories, 'subcategories' => $subcategories, 'subcategories_all' => $subcategories_all]);
    }
        ...
        $result = [];
                if(count($data) > 0){
                        ...
                        $query = DB::table('money_transfers as mt')
                                ...
                                ->select('mt.*', 'fa.name as fa_name', 'fat.name as fat_name', 
                                'users.first_name as user_name' , 'users.last_name as user_name2',
                                'accounts.name as account_name', 'em.first_name as employee_name', 
                ...
                return view('advancereports.cash_count_transfers', ['result' => $result, 'stores' => $stores, 
        ...
        $result = [];
                if(count($data) > 0){
                        ...
                        $query = DB::table('money_transfers as mt')
                                ...
                                ->leftJoin('accounts', 'accounts.id', '=', 'mt.from_account_id')
                                ->leftJoin('employees as em', 'em.id', '=', 'mt.real_employee_id')
                                ->select('fa.name as fa_name', 'fat.name as fat_name',
                                'users.first_name as user_name' , 'users.last_name as user_name2'
                                ,'mt.document_number as number', 'mt.document_date as created_at',
                                'mt.completed as completed', 'mt.description as description',
                                'mt.cash_count_id as cash_count_id', 'mt.cash_count_out_id as cash_count_out_id',
                                'mt.amount as amount', 'mt.id as id', 'accounts.name as account_name', 
                                'em.first_name as employee_name', 'em.last_name as employee_name2'
                                ) 
                                ->whereDate('mt.created_at', '>=', $from_date)
                                ->whereDate('mt.created_at', '<', $to_date)
                        ...
                        $query2 = DB::table('payments as py')
                                ...
                                ->select('pt.name as fat_name',
                                'users.first_name as user_name' , 'users.last_name as user_name2',
                                'inv.invoice_number as number', 'py.payment_date as created_at',
                                'py.cash_count_id as cash_count_id','py.amount as amount', 'py.id as id',
                                'py.payment_status_id as completed', 'accounts.name as account_name',
                                'em.first_name as employee_name', 'em.last_name as employee_name2'
                                ) 
                        ...
                        $query3 = DB::table('money_incomes as mi')
                                ...
                                ->select('pt.name as fat_name',
                                'users.first_name as user_name' , 'users.last_name as user_name2',
                                'mi.document_number as number', 'mi.document_date as created_at',
                                'mi.description as description',
                                'mi.cash_count_id as cash_count_id','mi.amount as amount', 'mi.id as id',
                                'mi.id as completed', 'accounts.name as account_name', 'ic.name as fa_name',
                                'em.first_name as employee_name', 'em.last_name as employee_name2'
                                ) 
                        ...
                        $query4 = DB::table('store_credits as sc')
                                ...
                                ->select('pt.name as fat_name',
                                'users.first_name as user_name' , 'users.last_name as user_name2',
                                'sc.credit_number as number', 'sc.credit_date as created_at',
                                'sc.private_notes as description',
                                'sc.cash_count_id as cash_count_id','sc.amount as amount', 'sc.id as id',
                                'sc.is_approved as completed', 'accounts.name as account_name', 'sc.id as store',
                                'em.first_name as employee_name', 'em.last_name as employee_name2'
                                ) 
                ...
                return view('advancereports.cash_count_bank_transfers', ['result' => $result, 'stores' => $stores, 
                'financeAccounts' => $financeAccounts, 'paymentTypes' => $paymentTypes]);
    }
        ...
        $result = [];
                if(count($data) > 0){
                        ...
                        $query = DB::table('payments as py')
                                ...
                                ->select('pt.name as fat_name',
                                'users.first_name as user_name' , 'users.last_name as user_name2',
                                'inv.invoice_number as number', 'py.payment_date as payment_date',
                                'py.cash_count_id as cash_count_id', 'py.id as id',
                                'py.payment_status_id as payment_status_id', 'inv.discount_percent as discount_percent',
                                'inv.discount_points as discount_points','inv.discount_vouchers as discount_vouchers',
                                'inv.discount as discount','inv.amount as amount','inv.total as total',
                ...
                return view('advancereports.cash_count_sales', ['result' => $result, 'stores' => $stores, 
        ...
        $result = [];
                if(count($data) > 0){
                        ...
                        $query = DB::table('money_incomes as mi')
                                ...
                                ->select('pt.name as py_name',
                                        'users.first_name as user_name' , 'users.last_name as user_name2',
                                        'mi.document_number as number', 'mi.created_at as created_at',
                                        'mi.cash_count_id as cash_count_id', 'mi.id as id',
                                        'accounts.name as account_name', 'ic.name as ca_name', 
                                        'mi.description', 'mi.amount', 'cc.cash_count_date as asig_date',
                                        'em.first_name as employee_name', 'em.last_name as employee_name2'
                                ) 
                                ->whereDate('cc.cash_count_date', '>=', $from_date)
                                ->whereDate('cc.cash_count_date', '<', $to_date)
                ...
                return view('advancereports.cash_count_incomes', ['result' => $result, 'stores' => $stores, 
                'paymentTypes' => $paymentTypes]);
    }
    ...
    public function routes(Request $request){
        ...
        $result = [];
                if(count($data) > 0){
                        ...
                        if($type == "contado"){
                                $query = DB::table('invoices as inv')
                                        ...
                                        ->select('pt.name as fat_name',
                                        'users.first_name as user_name' , 'users.last_name as user_name2',
                                        'inv.invoice_number as number', 'inv.date_changed_credit as payment_date',
                                        'inv.cash_count_id as cash_count_id', 'inv.id as id',
                                        'py.payment_status_id as payment_status_id', 'inv.discount_percent as discount_percent',
                                        'inv.discount_points as discount_points','inv.discount_vouchers as discount_vouchers',
                                        'inv.discount as discount','inv.amount as amount','inv.balance as balance',
                                        'accounts.name as account_name', 'ro.name as route_name' ,
                        ...
                        elseif($type == "credito"){
                                $query = DB::table('invoices as inv')
                                        ...
                                        'users.first_name as user_name' , 'users.last_name as user_name2',
                                        'inv.invoice_number as number', 'inv.date_changed_credit as payment_date',
                                        'cc.id as cash_count_id', 'inv.id as id',
                        ...
                        elseif($type == "abono"){
                                ...
                                ->select('pt.name as fat_name',
                                        'users.first_name as user_name' , 'users.last_name as user_name2',
                                        'mi.document_number as number', 'mi.created_at as payment_date',
                                        'mi.cash_count_id as cash_count_id', 'mi.id as id',
                                        'accounts.name as account_name', 'mi.amount as amount',
                                        'ic.name as ca_name','mi.description', 'ro.name as route_name' 
                                ) 
                        ...
                        elseif($type == "otros"){
                                ...
                                ->select('pt.name as fat_name',
                                        ...
                                        'mi.cash_count_id as cash_count_id', 'mi.id as id',
                                        'accounts.name as account_name', 'mi.amount as amount',
                                        'ic.name as ca_name','mi.description'
                                ) 
                ...
                return view('advancereports.cash_count_routes', ['result' => $result, 'stores' => $stores, 
                'paymentTypes' => $paymentTypes, 'data'=>$data]);
    }
    ...
    public function salesBySac(Request $request){
		...
		$invoices = DB::table('invoices')->join('clients', 'invoices.client_id', '=', 'clients.id')
				...
				->select('invoices.total_cost AS total_cost', 'invoices.amount AS amount', 
                                ...
                                'clients.name', 'clients.address1', 'clients.work_phone', 'clients.phone', 
                                'clients.id', 'clients.type', 'invoices.seller_id', 'invoices.auxiliar_id', 'invoices.invoice_date as invoice_date', 'accounts.name as account_name')
				->where('accounts.exclude', 0)
    ...
    public function per_diem(Request $request){
        ...
        if(isset($data['export']) && $data['export'] == 1){
                ...
                $columns = [
                        'ID', 'First Name', 'Last Name', 'Status', 'Total Amount', 
                        'Total Amount Paid', 'Total Declared', 'Total Cuadre', 
                        'Created At', 'Date Paid', 'User Create', 'User Approved', 
                        'User Paid', 'General Comments'
                ];
            ...
            foreach($tracesRequests as $tracesRequest){
                foreach($tracesRequest->items as $item){
                    $result = [
                        ...
                        'priority' => ($item->priority == 0 || trim($item->priority) == '') ? 'Por Asignar' :
    ...
    public function cashCountNetSales(Request $request){
        ...
        if(count($data) > 0){
            ...
            $cashcounts = CashCount::
                select(['id','account_id', 'cash_count_date', 'total_sales', 'refunds_cash'])
    ...
    public function stock_in_stores(Request $request){
        ...
        if(count($data) > 0){
            ...
            $nameFile = 'inventario_tiendas_total'.$currentDate[0].$currentTime.'.csv';
            ...
            $reportProcess->save();
            $reportProcessId = $reportProcess->id;
            dispatch((new ReportStockInStores($nameFile, $reportProcessId, $currentStores))->delay(15));
    ...
    public function stock_in_stores_detalle(Request $request){
        $data = $request->all();
        $stores = Account::where('exclude', 0)->get()->keyBy('id');
        $nameReport = 'inventario_tiendas_detalle';
        ...
        if(count($data) > 0){
            ...
            $nameFile = $nameReport.$currentDate[0].$currentTime.'.csv';
            ...
            $reportProcess = new ReportProcess;
            $reportProcess->file = $nameFile;
            $reportProcess->report = $nameReport;
            $reportProcess->status = 0;
            $reportProcess->count_rows = 0;
            $reportProcess->rows = ($rows == 1) ? $rows : intval(ceil($rows / 4));
            $reportProcess->save();
            ...
            if($rows == 1){
                dispatch((new ReportStockInStoresDetalle($nameFile, $reportProcessId, $currentStores))->delay(15));
            }else{
                $count = 1;
                foreach (array_chunk($currentStores, 4) as $chunkStores){
                    dispatch((new ReportStockInStoresDetalle($nameFile, $reportProcessId, $chunkStores))->delay(15 * $count));
                    $count = $count+1;
                };
            };

        }
        $reportProcess = ReportProcess::where('report',$nameReport)->orderBy('id', 'DESC')->take(30)->get();
        return view('advancereports.stock_in_stores_detalle', ['reportProcess' => $reportProcess,  'stores' => $stores]);
    }
    ...
    public function commission_old_products(Request $request){
        ...
        if(count($data) > 0){
            ...
            $reportProcessId = $reportProcess->id;
            if($rows == 1){
                dispatch((new ReportCommissionOldProducts($nameFile, $reportProcessId, $currentStores, $from_date, $to_date))->delay(15));
            }else{
                $count = 1;
                 foreach (array_chunk($currentStores, 4) as $chunkStores){
                    dispatch((new ReportCommissionOldProducts($nameFile, $reportProcessId, $chunkStores, $from_date, $to_date))->delay(15 * $count));
                    $count = $count+1;
                };
            };
        }
    ...
    public function invoices_deleted(Request $request){
        ...
        if(count($data) > 0){
            ...
            $items = DB::table('invoices')
				...
				->select('invoice_items.product_key', 'invoice_items.notes','invoices.deleted_at', 
                'invoice_items.qty', 'invoice_items.cost', 'invoices.invoice_number', 
                'invoices.invoice_date', 'accounts.name as account', 'users.first_name', 
                'users.last_name', 'clients.name as client', 'invoices.employee_id', 'accounts.id as account_id')
    ...
    public function SubcategoryGroupByCategory(Request $request){
        ...
        if(count($data) > 0){
            ...
            $invoices = DB::table('invoices')
                ...
                ->join('accounts', 'accounts.id', '=', 'invoices.account_id')
                ->select(
                    'invoice_items.qty', 'invoice_items.cost', 'products.category_id',
                    'products.sub_category_id', 'products.rotation_id'
                )
                ->where('accounts.exclude', 0)
                ->whereNull('invoice_items.deleted_at')
                ->where('invoices.invoice_type_id', 1)
        ...
        if(count($data) > 0){
                ...
                if($withTracking === true){
                    ...
                    $tracking = DB::table('products_tracking')
                        ->select(
                            ...
                            'products_tracking.original_account_id', 'products_tracking.final_account_id',
                            'products_tracking.original_quantity_after', 'products_tracking.final_quantity_after',
            ...
            $products = DB::table('products')
                ...
                ->select(
                    ...
                    'products.category_id', 'products.sub_category_id',
