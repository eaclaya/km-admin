...
class Payment extends ModelDBMain
{
    ...
    {
        return $this->belongsTo('App\Models\Main\Invitation');
    }
    ...
    {
        return $this->belongsTo('App\Models\Main\Contact');
    }
    ...
    {
        return $this->belongsTo('App\Models\Main\AccountGateway')->withTrashed();
    }
    ...
    {
        return $this->belongsTo('App\Models\Main\PaymentMethod');
    }
    ...
    {
        return $this->belongsTo('App\Models\Main\PaymentStatus');
    }
...
Payment::created(function ($payment) {
    ...
    if(!Session::get('created_payment_'.$payment->invoice->invoice_number)){
        if ($payment->invoice->isType(INVOICE_TYPE_STANDARD)) {
            event(new CheckPromotionsActive($payment->invoice));
            if($payment->invoice->account_id !== 17){
                $randNumber = rand(1, 60);
                dispatch((new SentApiWhatsapp('WhatsappInvoiceWasCreated', $payment->invoice))->delay($randNumber));
            }
        }
    }
    Session::flash('created_payment_'.$payment->invoice->invoice_number, 1);
});
